            ...
            try {
                ...
                while (true) {
                    if (scanner.hasNextLine()) {
                        ...
                        Command cmd = null;
                        if (Arrays.asList(ObjectCommands).contains(commandType)) {
                            Movie movie = ElementInputManager.getMovie();
                            String arg = tokens.length > 1 ? tokens[1] : null;
                            cmd = new Command(commandType, arg, movie);
                        } else {
                            String arg = tokens.length > 1 ? tokens[1] : null;
                            cmd = new Command(commandType, arg, null);
                        }
